...
SELECT
  ...
  CASE
    WHEN de.event_distance_length <= 50 THEN 1
    WHEN de.event_distance_length <= 100 THEN 2
    WHEN de.event_distance_length <= 200 THEN 3
    ELSE 4
  END AS bucket_order,
  AVG(r.athlete_performance) AS avg_time
FROM
  marathos.gold.dim_event as de
    LEFT JOIN marathos.gold.fct_results AS r
      ON r.event_id = de.event_id
WHERE
  de.event_type = 'distance'
GROUP BY
  distance_bucket,
  bucket_order
ORDER BY
